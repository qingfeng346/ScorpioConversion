        ...
        void CheckData() {
            ...
            for (var i = 0; i < PackageClass.Fields.Count; ++i) {
                ...
                if (!field.Name.IsEmptyString() && field.Type.IsEmptyString()) {
                    throw new System.Exception($"列:{(i + 1).GetLineName()}({field.Name}) 类型为空");
                } else if (field.Name.IsEmptyString() && !field.Type.IsEmptyString()) {
                    throw new System.Exception($"列:{(i + 1).GetLineName()}名字为空,类型为{field.Type}");
                } else if (field.IsL10N && (!field.IsString || field.IsArray)) {
                    throw new System.Exception($"列:{(i + 1).GetLineName()}({field.Name}) L10N字段只支持string类型");
                } else if (fieldNames.Contains(field.Name)) {
                    throw new System.Exception($"列:{(i + 1).GetLineName()}({field.Name}) 有重复的字段名");
                }
