...
namespace Scorpio.Conversion {
    ...
    public class PackageParser {
        ...
        void ParseClass(string name, ScriptMap table) {
            ...
            foreach (var pair in table) {
                var fieldName = pair.Key as string;
                if (string.IsNullOrEmpty(fieldName)) throw new System.Exception($"Class:{name} Field:{fieldName} 参数出错 参数模版 \"[索引],[类型],[是否数组=false],[默认值],[注释]\"");
                var value = pair.Value.ToString();
                var infos = value.Split(',');
                if (infos.Length < 2) throw new System.Exception($"Class:{name} Field:{fieldName} 参数出错 参数模版 \"[索引],[类型],[是否数组=false],[默认值],[注释]\"");
