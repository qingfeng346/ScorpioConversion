...
namespace Scorpio.Conversion.Engine {
    public class Config {
        ...
        public static List<string> MergeFiles { get; private set; }                 //所有配置文件
        ...
        public static void Initialize(string[] configs, string[] files, string[] paths, string[] mergeFiles, string[] mergePaths, string[] tags, string[] branches, string info) {
            ...
            MergeFiles.AddRange(mergeFiles);
            foreach (var path in mergePaths) {
                MergeFiles.AddRange(Directory.GetFiles(path, "*", SearchOption.AllDirectories).Where(_ => _.IsExcel()));
            }
            if (info.EndsWith(".yaml")) {
                BuildInfo = new DeserializerBuilder().WithNamingConvention(CamelCaseNamingConvention.Instance).Build().Deserialize<BuildInfo>(FileUtil.GetFileString(info));
            } else {
