...
const DataTestCsv = require("./DataTestCsv")
class TableTestCsv {
    constructor() {
        this.m_count = 0;
    ...
    Initialize(fileName, reader) {
        ...
        if ("f07d3fff17de6b37025a951b272f6c4c" != reader.ReadString()) {
            throw new Error("File schemas do not match [TableTestCsv] : ${fileName}");
        }
        reader.ReadHead();
        for (let i = 0; i < row; i ++) {
            let pData = new DataTestCsv(fileName, reader);
